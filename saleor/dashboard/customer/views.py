...
def customer_list(request):
    customers = (
        ...
        .select_related("default_address", "default_address")
        .order_by("email")
    )
...
def ajax_users_list(request):
    queryset = User.objects.select_related("default_address")
    search_query = request.GET.get("q", "")
    if search_query:
        queryset = queryset.filter(
            Q(default_address__first_name__icontains=search_query)
            | Q(default_address__last_name__icontains=search_query)
...
def customer_delete(request, pk):
    ...
    return TemplateResponse(
        request, "dashboard/customer/modal/confirm_delete.html", ctx, status=status
    )
