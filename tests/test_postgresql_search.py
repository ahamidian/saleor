...
def test_find_product_by_name(admin_client, named_products, phrase, product_num):
...
def test_find_product_by_description(admin_client, named_products, phrase, product_num):
    products, _, _ = search_dashboard(admin_client, phrase)
    assert 1 == len(products)
    assert named_products[product_num] in products
...
def orders_with_addresses():
    ...
    for pk, name, lastname, email in ORDERS:
        ...
        user = User.objects.create(default_address=addr, email=email)
...
def test_find_order_with_email(admin_client, orders_with_addresses, phrase, order_num):
    _, orders, _ = search_dashboard(admin_client, phrase)
    assert 1 == len(orders)
...
def test_find_order_with_user_name(
    admin_client, orders_with_addresses, phrase, order_num
):
    _, orders, _ = search_dashboard(admin_client, phrase)
    assert 1 == len(orders)
    assert orders_with_addresses[order_num] in orders
...
def test_find_user_by_email(admin_client, users_with_addresses, phrase, user_num):
...
def test_find_user_by_name(admin_client, users_with_addresses, phrase, user_num):
    _, _, users = search_dashboard(admin_client, phrase)
    assert 1 == len(users)
    assert users_with_addresses[user_num] in users
...
def test_find_user_by_name_without_address(
    admin_client, users_with_names, phrase, user_num
):
    _, _, users = search_dashboard(admin_client, phrase)
    assert 1 == len(users)
