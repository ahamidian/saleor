...
@pytest.mark.integration
@pytest.mark.vcr(record_mode="once", match_on=MATCH_SEARCH_REQUEST)
def test_dashboard_search_with_product_result(db, indexed_products, admin_client):
    products, _, _ = execute_dashboard_search(admin_client, PHRASE_WITH_RESULTS)
...
ORDERS = {USER_EMAIL_WITH_ORDER: {11, 17}, USER_EMAIL_WITH_ORDER_WITHOUT_ADDRES: {15}}
...
@pytest.mark.integration
@pytest.mark.vcr(record_mode="once", match_on=MATCH_SEARCH_REQUEST)
def test_dashboard_search_user_name_without_address(db, admin_client, customers):
    _, users, _ = execute_dashboard_search(admin_client, EXISTING_NAME_WITHOUT_ADDRESS)
...
@pytest.mark.integration
@pytest.mark.vcr(record_mode="once", match_on=MATCH_SEARCH_REQUEST)
def test_dashboard_search_orders_by_user_email(db, admin_client, customers, orders):
    _, _, orders = execute_dashboard_search(admin_client, USER_EMAIL_WITH_ORDER)
    assert ORDERS[USER_EMAIL_WITH_ORDER] == orders


@pytest.mark.integration
@pytest.mark.vcr(record_mode="once", match_on=MATCH_SEARCH_REQUEST)
def test_dashboard_search_orders_by_user_name(db, admin_client, customers, orders):
    _, _, orders = execute_dashboard_search(admin_client, USER_NAME_WITH_ORDER)
    assert ORDERS[USER_EMAIL_WITH_ORDER] == orders


@pytest.mark.integration
@pytest.mark.vcr(record_mode="once", match_on=MATCH_SEARCH_REQUEST)
def test_dashboard_search_orders_by_user_name_without_address(
    db, admin_client, customers, orders
):
